...
4. Update `lib/main_dev.dart` and `lib/main_prod.dart` (`appName`, `apiBaseUrl`, `deepLinkHost`, `appStoreId` for prod when known, and `authEndpoints` when the backend paths differ from the defaults), the default `deepLinkScheme` in `lib/core/config/app_config.dart`, and the Android custom scheme in `AndroidManifest.xml`. Reword the iOS usage descriptions in `ios/Runner/Info.plist` and update the pinning placeholder in `android/app/src/main/res/xml/network_security_config.xml` if pinning is enabled.
